"""
Negative control for the Flickr8k open-vocabulary experiment (Section 4.5),
addressing the peer review finding that the closed-set negative-control
rigor (Section 4.3) was never applied to the open-vocabulary experiment.
Each image is paired during training with a real caption belonging to a
different, randomly chosen image (never its own), while evaluation still
measures retrieval against the image's TRUE captions. If real Flickr8k
accuracy is an artifact of some exploitable regularity independent of true
pairing, mismatched-pair training should perform similarly to matched
training. If it collapses toward chance, the real result reflects genuine
cross-modal alignment.

Usage: python run_openvocab_negative_control.py <objective> <seed>
"""
import json
import os
import re
...
print(f"[negctrl {OBJECTIVE} seed {SEED}] Device: {DEVICE}", flush=True)
...
class MismatchedFlickr8kPairs(torch.utils.data.Dataset):
    """NEGATIVE CONTROL: each image is paired with a real caption drawn
    from a DIFFERENT, randomly chosen image (never its own)."""
    ...
    def __getitem__(self, idx):
        ...
        wrong_idx = idx
        while wrong_idx == idx:
            wrong_idx = int(self.rng.integers(0, len(self.files)))
        wrong_fname = self.files[wrong_idx]
        caps = self.captions_by_img[wrong_fname]
...
def main():
    print(f"\n=== [negctrl {OBJECTIVE} seed {SEED}] Loading real Flickr8k (MISMATCHED pairs) ===", flush=True)
    ...
    train_ds = MismatchedFlickr8kPairs(train_files, captions_by_img, vocab, seed=SEED)
    ...
    EPOCHS = 15
    print(f"\n=== [negctrl {OBJECTIVE} seed {SEED}] Training on MISMATCHED pairs, {EPOCHS} epochs ===", flush=True)
    t0 = time.time()
    for epoch in range(EPOCHS):
        ...
        print(f"  [negctrl {OBJECTIVE} seed {SEED}] epoch {epoch+1}: loss={total_loss/n_batches:.4f}", flush=True)
    train_time = time.time() - t0

    print(f"\n=== [negctrl {OBJECTIVE} seed {SEED}] Eval: retrieval against TRUE captions ===", flush=True)
    ...
    print(f"  [negctrl {OBJECTIVE} seed {SEED}] Recall@1={r1:.4f} R@5={r5:.4f} R@10={r10:.4f} "
          f"(real matched-pair result for comparison, see openvocab_{OBJECTIVE}_seed0_results.json)", flush=True)

    result = {
        "objective": OBJECTIVE, "seed": SEED, "condition": "mismatched_pairs_negative_control",
    ...
    out_file = f"openvocab_negctrl_{OBJECTIVE}_seed{SEED}_results.json"
