"""
Embedding-dimension ablation on the domain-controlled cardinality-100 test
(run_cardinality_ablation_v2.py). Tests Hypothesis A for why Barlow Twins
loses to InfoNCE as cardinality grows: is it a capacity mismatch (fixed
embed_dim=64 giving too few degrees of freedom to separate 100 classes),
rather than a deeper mechanism-level failure of the correlation objective?

Identical to run_cardinality_ablation_v2.py in every respect except
EMBED_DIM is now a CLI argument instead of hardcoded to 64.

Usage: python run_embeddim_ablation.py <objective> <seed> <embed_dim>
"""
import json
import math
import sys
...
print(f"[embeddim {OBJECTIVE} seed {SEED} dim {EMBED_DIM}] Device: {DEVICE}", flush=True)
...
class ImageEncoder(nn.Module):
    ...
    same capacity, only embed_dim (and cardinality) changes."""
...
def main():
    print(f"\n=== [embeddim {OBJECTIVE} seed {SEED} dim {EMBED_DIM}] Loading real MNIST (100 pseudo-classes) ===", flush=True)
    ...
    EPOCHS = 20
    print(f"\n=== [embeddim {OBJECTIVE} seed {SEED} dim {EMBED_DIM}] Training, {EPOCHS} epochs, {NUM_CLASSES} classes ===", flush=True)
    t0 = time.time()
    for epoch in range(EPOCHS):
        ...
        print(f"  [embeddim {OBJECTIVE} seed {SEED} dim {EMBED_DIM}] epoch {epoch+1}: loss={total_loss/n_batches:.4f} "
              f"({time.time()-t0:.0f}s elapsed)", flush=True)
    train_time = time.time() - t0

    print(f"\n=== [embeddim {OBJECTIVE} seed {SEED} dim {EMBED_DIM}] Eval ===", flush=True)
    ...
    print(f"  [embeddim {OBJECTIVE} seed {SEED} dim {EMBED_DIM}] linear_probe={probe_acc:.4f} top1={top1_correct:.4f} top5={top5_correct:.4f} "
          f"(chance: probe={1/NUM_CLASSES:.4f}, top1={1/NUM_CLASSES:.4f}, top5={5/NUM_CLASSES:.4f})", flush=True)

    record = {
        "objective": OBJECTIVE, "seed": SEED, "num_classes": NUM_CLASSES, "embed_dim": EMBED_DIM,
        "design": "MNIST digit x rotation-bucket pseudo-classes (domain-controlled), embed_dim ablation",
    ...
    out_file = f"embeddim_{OBJECTIVE}_seed{SEED}_dim{EMBED_DIM}_results.json"
